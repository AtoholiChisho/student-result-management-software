...
class Register:
    def __init__(self,root):
        ...
        #======bg image=====
        self.bg=ImageTk.PhotoImage(file="images/registerbg2.jpg")
        bg=Label(self.root,image=self.bg).place(x=0,y=0,relwidth=1,relheight=1)

        #======LEFT image=====
        self.left=ImageTk.PhotoImage(file="images/hisho.jpg")
        ...
        self.cmb_quest['values']=("select","Your Birth Time","Your Favourite Movie","Your First Korean Drama")
        ...
        btn_register=Button(frame1,text="Register Now",font=("times new roman",15),bg="Green",activebackground="Green",cursor="hand2",command=self.register_data).place(x=220,y=430,width=250,height=35)

        btn_login=Button(self.root,text="Sign In",font=("times new roman",20),bd=0,cursor="hand2",command=self.login_window).place(x=190,y=490,width=180)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                row=cur.fetchone()
                if row!=None:
                     messagebox.showerror("Error","User already Exists.Please try with a different Email",parent=self.root)
                else:
